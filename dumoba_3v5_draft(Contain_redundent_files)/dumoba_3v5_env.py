...
class Dumoba3v5Env(gym.Env):
    """
    Dumoba 3v5: Blue controls 3 heroes (RL), red has 5 stronger scripted heroes.
    Each red hero is allowed to respawn once at its original spawn position.
    """
    ...
    def __init__(self, render_mode: str | None = None, max_steps: int = 200):
        ...
        # HP and damage config
        self.max_blue_hero_hp = 10
        self.max_red_hero_hp = 12   # buffed red heroes
        self.max_base_hp = 20

        self.blue_attack_damage = 3
        self.red_attack_damage = 5  # buffed red damage

        # Each red hero can respawn once
        ...
        # Bases: blue at top center, red at bottom center
    ...
    def reset(self, seed: int | None = None, options=None):
        ...
        # Spawn 5 red heroes near red base in a small block and store spawn positions
        cx_r = self.red_base_pos[0]
        bottom_row = self.red_base_pos[1]
        red_row1 = min(self.grid_size - 1, bottom_row + 1)
        red_row2 = min(self.grid_size - 1, bottom_row + 2)
        red_positions = [
            (cx_r - 1, red_row1),
            (cx_r,     red_row1),
            (cx_r + 1, red_row1),
        ...
        self.red_spawn_positions = [
            np.array(p, dtype=np.int32) for p in red_positions
        ]
    ...
    def step(self, action):
        ...
        # Handle red respawns AFTER damage is resolved
    ...
    def _scripted_policy(self, agent_id: int) -> np.ndarray:
        """Very simple scripted policy: move toward nearest enemy or base, attack if in range."""
    ...
    def _check_done(self):
        ...
        # Red: still "in game" if alive OR has respawns left
...
if __name__ == "__main__":
    # Quick manual test
    env = Dumoba3v5Env(render_mode="human")
    obs, info = env.reset()
    done = False
    truncated = False
